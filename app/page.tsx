  ...
  return (
    <div className={`min-h-screen p-8  ${currentTheme.transitions.page} ${currentTheme.page.bgColour} ${currentTheme.page.textColour} `}>
      ...
      <footer className={`mt-8 p-6 text-center rounded-lg ${currentTheme.transitions.element} ${currentTheme.summary.bgColour}`}>
        <p className='text-xs opacity-30 text-neutral-300'>Data source:</p>
        <p className='text-sm opacity-80 text-neutral-200'>{data.metadata.data_source}</p>
        <span className='text-xs opacity-30 text-neutral-300'>Accessed on:</span>
        <p className='text-xs text-neutral-200 '>{new Date(data.metadata.accessed_on).toLocaleString('en-GB', options)}</p>
        <span className='text-xs opacity-30 text-neutral-300'>Generated by:</span>
        <p className='text-xs opacity-60 text-neutral-100'>Awareness Engine v0.0.1</p>
        <p className='text-xs opacity-30 text-neutral-300 mt-2'>Current Theme: {currentTheme.name}</p>
      </footer>
    </div>
  );
}
